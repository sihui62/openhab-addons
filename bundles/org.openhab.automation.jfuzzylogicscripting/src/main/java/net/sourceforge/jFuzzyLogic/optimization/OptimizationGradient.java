 ...
 * Gradient optimization: Simply calculates gradient and tries to optimize in that direction using a 'line search'
 * algorithm.
 * 
...
public class OptimizationGradient extends OptimizationPartialDerivate {
    ...
    public OptimizationGradient(RuleBlock fuzzyRuleSet, ErrorFunction errorFunction,
    ...
    private double gradient(double gradient[], double ev0) {
        ...
        for (int i = 0; i < parameterList.size(); i++) {
            ...
            if (param.set(paramValue + epsilon))
                ...
                epsilon = 0; // Cant' increase / decrease => no change
            ...
            if (verbose)
                System.out.println(
                        "Parameter:" + param.getName() + "\tDerivate: " + gradient[i] + "\tepsilon: " + epsilon);
    ...
    public void optimize() {
        ...
        for (i = 0; i < maxIterations; i++) {
            ...
            if (verbose)
                System.out.println("Iteration: " + i + "\tError: " + error0);
            ...
            if (norm <= EPSILON) {
                ...
                if (verbose)
                    System.out.println("Gradient's norm too small => finished (norm: " + norm + ")");
                return;
            }
            ...
            for (k = 0; k < maxLineSearchIterations; k++) {
                ...
                if (verbose)
                    System.out.println(
                            "\tLine tteration: " + k + "\talpha: " + alpha + "\tDelta_Error: " + (error - error0));
                if (error < error0) {
