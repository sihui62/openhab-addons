...
public abstract class RuleActivationMethod extends FclObject {
    ...
    public RuleActivationMethod(String name) {
        this.name = name;
    }
    ...
    public void imply(RuleTerm fuzzyRuleTerm, RuleAccumulationMethod ruleAccumulationMethod, double degreeOfSupport) {
        ...
        Objects.requireNonNull(defuzzifier);
        ...
        if (mf.isDiscrete() != defuzzifier.isDiscrete()) {
            throw new RuntimeException(
                    "MembershipFunction and Defuzzifier are neither both discrete nor both continuous\n\tTerm: "
        ...
        if (mf.isDiscrete()) {
            ...
            if (Double.isNaN(x) || Double.isInfinite(x)) {
                throw new RuntimeException(
                        "Universe minimum not calculated for term '" + fuzzyRuleTerm.getTermName() + "' : " + x);
            }
            if (Double.isNaN(step) || Double.isInfinite(step)) {
                throw new RuntimeException(
                        "Step not calculated for term '" + fuzzyRuleTerm.getTermName() + "' : " + step);
            }
