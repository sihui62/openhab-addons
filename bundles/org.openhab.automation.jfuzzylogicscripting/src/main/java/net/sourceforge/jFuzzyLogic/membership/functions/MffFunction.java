    ...
    protected void evaluateTerms() {
        ...
        for (int i = 0; i < terms.length; i++) {
            if (terms[i] == null) { // Null function => Nothing to do
                ...
                if (var.isOutput()) {
                    throw new RuntimeException("Can't use an output variable '" + var.getName()
                            + "' for a function (It may create a race condition)");
                }
                ...
                if (debug) {
                    Gpr.debug("Evaluated: " + mff + " = " + values[i]);
                }
            }
            if (debug) {
                Gpr.debug("Term[" + i + "]: " + terms[i] + "\tClass:" + terms[i].getClass().getSimpleName()
                        + "\tValue: " + values[i]);
            }
        }
    }
        ...
        if (terms != null) {
            for (FclObject fco : terms) {
                if (fco instanceof Variable) {
                    ...
                    Value val = (Value) fco;
                    if (val.getType() == Value.Type.VAR_REFERENCE && val.getVarRef() != null) {
                        vars.add(val.getVarRef());
                    }
