...
public class MembershipFunctionPieceWiseLinear extends MembershipFunctionContinuous {
        ...
        if (x.length > 1) {
            for (int i = 0; i < x.length; i++) {
                ...
                if ((y[i].getValue() < 0) || (y[i].getValue() > 1)) {
                    ok = false;
                    if (errors != null) {
                        errors.append("Membership funcion out of range: y[" + i + "] = " + y[i]
    ...
    public double membership(double in) {
        ...
        if (in > x[len - 1].getValue()) {
            return y[len - 1].getValue();
        }
        for (i = 1; i < len; i++) {
            if (in <= x[i].getValue()) {
                return y[i - 1].getValue() + (y[i].getValue() - y[i - 1].getValue())
                        * ((in - x[i - 1].getValue()) / (x[i].getValue() - x[i - 1].getValue()));
            }
        }
        if (Double.isNaN(in)) {
            return Double.NaN;
