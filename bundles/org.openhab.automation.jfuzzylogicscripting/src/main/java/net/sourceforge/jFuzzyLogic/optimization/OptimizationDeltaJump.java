...
public class OptimizationDeltaJump extends OptimizationMethod {
    ...
    public OptimizationDeltaJump(RuleBlock fuzzyRuleSet, ErrorFunction errorFunction,
    ...
    public void optimizeIteration(int iterationNum) {
            ...
            if (jump != 0) { // Can jump?
                ...
                for (lineIterNum = 0; lineIterNum < maxLineSearchIterations; lineIterNum++)
                    ...
                    if (parameter.set(paramValue + alpha * jump)) { // Can set parameter?
                        ...
                        if (verbose)
                            System.out.println("\t\tLine iteration:" + lineIterNum + ", alpha:" + alpha
                                    + ", Delta_Error:" + deltaErr);
                        ...
                        break; // Cannot set parameter? => break loop
                ...
                if (verbose) {
                    if (lineIterNum >= maxLineSearchIterations)
                        System.out.println(
                                "\t\tWARNING: Too many line iterations! This may point a desing error in your fuzzy system.");
                    if (paramValue != parameter.get())
                        System.out.println("\tIteration: " + iterationNum + "\tParameter: " + paramNum + " ("
                                + parameter.getName() + ")\tError before: " + error0 + "\tError after: "
                                + bestAlphaError + "\tParameter before: " + paramValue + "\t after: " + parameter.get()
        ...
        if (verbose)
            System.out.println("Delta error (full iteration): " + deltaErrorIt);
        if (deltaErrorIt >= 0) {
            if (verbose)
