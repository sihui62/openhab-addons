...
public abstract class Parameter {
    ...
    public static final double UNIVERSE_TO_EPSILON_RATIO = 1000; // How we get 'epsilon' (to be used to calculate
                                                                 // partial derivative) from universe

    private final double epsilon; // A small number (e.g used to calculate derivatives)
    private final String name; // Paramter's name
     ...
     */
    private static List<Parameter> parametersMembershipFunction(Variable variable) {
        double epsilon;
        ArrayList<Parameter> parameterList = new ArrayList<Parameter>();
        ...
        for (String termName : variable.getLinguisticTerms().keySet()) {
            ...
            if (delta == 0) {
                delta = variable.getUniverseMax() - variable.getUniverseMin();
            }
            ...
            for (int i = 0; i < membershipFunction.getParametersLength(); i++) {
                String paramName = variable.getName() + "_" + linguisticTerm.getTermName() + "_"
                        + membershipFunction.getName() + "_" + i;
                ParameterMembershipFunction param = new ParameterMembershipFunction(paramName, epsilon, variable,
     ...
     */
    private static List<Parameter> parametersRuleWeight(Rule fuzzyRule) {
        ArrayList<Parameter> parameterList = new ArrayList<Parameter>();
