 ...
 */
public class RuleBlock extends FclObject implements Iterable<Rule>, Comparable<RuleBlock> {

    FunctionBlock functionBlock; // Which FunctionBlock does this RuleBlock belong to
    String name;
    RuleAccumulationMethod ruleAccumulationMethod; // Rule accumulation method: How results of the rules are combined to
                                                   // obtain an overall result (e.g. MAX: maximum, BSUM: bounded sum,
                                                   // etc.)
    RuleActivationMethod ruleActivationMethod; // Rule activation (implication) method: How the 'if' activates the
                                               // 'then' (e.g. MIN: minimum, PROD: product)
    ...
    RuleAccumulationMethod createAccumulationMethod(String ruleAccumulationMethodType) {
        ...
        if (ruleAccumulationMethodType.equalsIgnoreCase("MAX")) {
            ...
            throw new RuntimeException(
                    "Unknown/Unimplemented Rule accumulation method '" + ruleAccumulationMethodType + "'");
        }
    ...
    public void evaluate() {
        // Apply each rule
        for (Rule rule : rules) {
            Gpr.debug("Evaluating rule: " + rule);
            rule.evaluate();
        }
    }
        ...
        for (int childNum = 1; childNum < tree.getChildCount(); childNum++) {
            ...
            if (leaveName.equalsIgnoreCase("AND")) {
                ...
                if (type.equalsIgnoreCase("MIN")) {
                    ...
                    throw new RuntimeException("Unknown (or unimplemented) 'AND' method: " + type);
                }
                ...
                String type = child.getChild(0).getText(); // Bug corrected by Arkadiusz M.
                                                           // amaterek@users.sourceforge.net
                if (type.equalsIgnoreCase("MAX")) {
                    ...
                    throw new RuntimeException("Unknown (or unimplemented) 'OR' method: " + type);
                }
            } else if (leaveName.equalsIgnoreCase("ACT")) {
            ...
            } else { // Accumulation method
                throw new RuntimeException("Unknown (or unimplemented) ruleblock item : " + leaveName);
            }
        }
    ...
    private void fclTreeRuleBlockActivation(Tree tree) {
        ...
        if (type.equalsIgnoreCase("MIN")) {
            ...
            throw new RuntimeException("Unknown (or unimplemented) 'ACT' method: " + type);
        }
    }
    ...
    private void fclTreeRuleBlockRule(Tree tree, RuleConnectionMethod and, RuleConnectionMethod or) {
        ...
        for (int childNum = 1; childNum < tree.getChildCount(); childNum++) {
            ...
            } else if (type.equalsIgnoreCase("THEN")) {
                fclTreeRuleBlockRuleThen(child, fuzzyRule);
            } else if (type.equalsIgnoreCase("WITH")) {
                fclTreeRuleBlockRuleWith(child, fuzzyRule);
            } else {
                throw new RuntimeException("Unknown (or unimplemented) rule block item: " + type);
            }
        }
    ...
    private void fclTreeRuleBlockRuleThen(Tree tree, Rule fuzzyRule) {
        ...
        for (int childNum = 0; childNum < tree.getChildCount(); childNum++) {
            ...
            if (variable == null) {
                throw new RuntimeException("Variable " + thenVariable + " does not exist");
            }
    ...
    public RuleBlock setVariable(String variableName, double value) {
        functionBlock.setVariableValue(variableName, value);
        return this;
    }
    ...
    public String toStringFcl() {
        ...
        if (ruleActivationMethod != null) {
            ruleBlockStr += "\t" + ruleActivationMethod.toStringFcl() + "\n";
        }
        if (ruleAccumulationMethod != null) {
            ruleBlockStr += "\t" + ruleAccumulationMethod.toStringFcl() + "\n";
        }
        if (!operator.isEmpty()) {
            ruleBlockStr += "\t" + operator + "\n";
        }
