...
public class MembershipFunctionGaussian2 extends MembershipFunctionContinuous {
    ...
    public boolean checkParamters(StringBuffer errors) {
        ...
        if (parameters[0].getValue() > parameters[2].getValue()) {
            ok = false;
            if (errors != null)
                errors.append("Parameters 'mean1' should be lower than 'meand2' : " + parameters[0] + " "
                        + parameters[2] + "\n");
        }
    ...
    public double membership(double in) {
        if (in < parameters[0].getValue()) {
            return Math.exp(-(in - parameters[0].getValue()) * (in - parameters[0].getValue())
                    / (2 * parameters[1].getValue() * parameters[1].getValue()));
        } else if (in > parameters[2].getValue()) {
            return Math.exp(-(in - parameters[2].getValue()) * (in - parameters[2].getValue())
                    / (2 * parameters[3].getValue() * parameters[3].getValue()));
        } else
            return 1;
    }
