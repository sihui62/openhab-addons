 ...
 * Derivate optimization: Simply calculates derivate and tries to optimize in that direction using a 'line search'
 * algorithm.
 *
...
public class OptimizationPartialDerivate extends OptimizationMethod {
    ...
    protected int countDerivatemTooSmall, countImpovement, countNoImpovement, countMaxIterations, countOptimizations,
            countIterations, countLineIterations, countGoodLineIterations, countBadLineIterations;
    ...
    public OptimizationPartialDerivate(RuleBlock fuzzyRuleSet, ErrorFunction errorFunction,
    ...
    private double derivate(int parameterNumber, double error0) {
        ...
        if (epsilon != 0) {
            ...
            error1 = error0; // No change

        // Partial derivate
        double derivate = (error0 - error1) / epsilon;
        if (error1 < error0)
            System.out.println("Can descent!   error1: " + error1 + "\terror0: " + error0);
        if (verbose)
    ...
    public void optimizeIteration(int iterationNum) {
        ...
        for (int paramNum = 0; paramNum < len; paramNum++) {
            ...
            if (Math.abs(derivate) <= EPSILON) {
                ...
                if (verbose)
                    System.out.println("Derivate's abs too small (derivate: " + derivate + " < "
                            + parameterList.get(paramNum).getEpsilon() + ")");
            } else {
                // ---
                ...
                for (int lineIterNum = 0; lineIterNum < maxLineSearchIterations; lineIterNum++) {
                    ...
                    if (verbose)
                        System.out.println("\tLine iteration: " + lineIterNum + "\talpha: " + alpha + "\tDelta_Error: "
                                + (error - error0));
                    if (error < error0) {
    ...
    public String stats(int iterationNumber) {
        String stats = "\tTotal Optimizations: " + countOptimizations//
                ...
                + "\n\tcountIterations: " + countIterations + "\tgood: " + countGoodLineIterations + " / bad: "
                + countBadLineIterations //
