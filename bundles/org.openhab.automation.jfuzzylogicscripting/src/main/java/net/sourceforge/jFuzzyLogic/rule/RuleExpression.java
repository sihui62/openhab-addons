 ...
 */
public class RuleExpression extends FclObject implements Iterable<Variable> {
     ...
     * @param ruleConnectionMethod : connection method between terms (which method is used for each 'and', 'or',
     *            'not'...)
     */
    ...
    public void add(RuleTerm fuzzyRuleTerm) {
        ...
        if (term1 == null) {
            setTerm1(fuzzyRuleTerm);
        } else if (term2 == null) {
            setTerm2(fuzzyRuleTerm);
        } else if (isFuzzyRuleExpression(term1)) {
            ((RuleExpression) term1).add(fuzzyRuleTerm);
        } else if (isFuzzyRuleExpression(term2)) {
            ((RuleExpression) term2).add(fuzzyRuleTerm);
        } else {
    ...
    private void addVariables(LinkedList<Variable> linkedListVariables) {
        ...
        if (isFuzzyRuleTerm(term1)) {
            linkedListVariables.add(((RuleTerm) term1).getVariable());
        } else if (isFuzzyRuleExpression(term1)) {
            ((RuleExpression) term1).addVariables(linkedListVariables);
        }

        // Term2: is it a 'term'? => add variables
        if (isFuzzyRuleTerm(term2)) {
            linkedListVariables.add(((RuleTerm) term2).getVariable());
        } else if (isFuzzyRuleExpression(term2)) {
            ((RuleExpression) term2).addVariables(linkedListVariables);
        }
    }
    ...
    public double evaluate() {
        ...
        if (isFuzzyRuleExpression(term1)) {
            resTerm1 = ((RuleExpression) term1).evaluate();
        } else if (isFuzzyRuleTerm(term1)) {
            resTerm1 = ((RuleTerm) term1).getMembership();
        } else if (term1 == null) {
            resTerm1 = Double.NaN;
        }

        // Evaluate term2: if it's an expression => recurse
        if (isFuzzyRuleExpression(term2)) {
            resTerm2 = ((RuleExpression) term2).evaluate();
        } else if (isFuzzyRuleTerm(term2)) {
            resTerm2 = ((RuleTerm) term2).getMembership();
        } else if (term2 == null) {
            resTerm2 = Double.NaN;
        }
    ...
    public List<RuleConnectionMethod> getRuleConnectionMethods() {
        ...
        if (isFuzzyRuleExpression(term1)) {
            rcons.addAll(((RuleExpression) term1).getRuleConnectionMethods());
        }
        if (isFuzzyRuleExpression(term2)) {
            rcons.addAll(((RuleExpression) term2).getRuleConnectionMethods());
        }
    ...
    public boolean isFuzzyRuleExpression(Object term) {
        ...
        if (term.getClass().getName().equals("net.sourceforge.jFuzzyLogic.rule.RuleExpression")) {
            return true;
        }
        return false;
    }
    ...
    public boolean isFuzzyRuleTerm(Object term) {
        ...
        if (term.getClass().getName().equals("net.sourceforge.jFuzzyLogic.rule.RuleTerm")) {
            return true;
        }
        return false;
    }
    ...
    public boolean isValidTerm(Object term) {
        ...
        if (isFuzzyRuleExpression(term)) {
            return true;
        }
        if (isFuzzyRuleTerm(term)) {
            return true;
        }
        return false;
    }
    ...
    public void setTerm1(FclObject term1) {
        if (!isValidTerm(term1)) {
            throw new RuntimeException(
                    "Invalid object for term1. Only 'RuleTerm' or 'RuleExpression' are accepted. Class: "
                            + term1.getClass().getName());
        }
        this.term1 = term1;
    }
    ...
    public void setTerm2(FclObject term2) {
        if (!isValidTerm(term2)) {
            throw new RuntimeException(
                    "Invalid object for term2. Only 'RuleTerm' or 'RuleExpression' are accepted. Class: "
                            + term2.getClass().getName());
        }
        this.term2 = term2;
    }
    ...
    public String toStringFcl() {
        ...
        if ((term1 == null) || (term2 == null)) {
            // Only one term?
            if (term1 != null) {
                str += term1.toStringFcl();
            }
            if (term2 != null) {
                str += term2.toStringFcl();
            }
        } else {
            // Both terms connected
            if (isFuzzyRuleExpression(term1)) {
                str += "(" + ((RuleExpression) term1).toStringFcl() + ")";
            } else if (isFuzzyRuleTerm(term1)) {
                str += ((RuleTerm) term1).toStringFcl();
            }

            str += connector;

            if (isFuzzyRuleExpression(term2)) {
                str += "(" + ((RuleExpression) term2).toStringFcl() + ")";
            } else if (isFuzzyRuleTerm(term2)) {
                str += ((RuleTerm) term2).toStringFcl();
            }
        }
